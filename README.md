...
### 15일차 : 2월 21일   
- 정렬
    - 퀵정렬 [노트북](./day15/da04_sort.ipynb)
    - 정렬 알고리즘 응용
        - 컬러이미지를 흑백이미지로 만들기 
            - rgb 이미지 ->그레이(중앙값) ->흑백(0,255)
            <img src='./image/cupdog_blackwhite.png' width=500>
        - 선택정렬 , 퀵정렬 비교
        - 버블정렬, 퀵정렬 비교
...
- 동적 계획법 
    - 등장 배경
        - 브루트 포스 검색 
            - 모든 경우의 수 다 확인해봄 -> 최선의 결과
        - 탐욕 알고리즘 
            - 가장 큰 이익의 데이터를 맨 먼저 선택하고 , 그다음 이익이 큰 데이터를 취하는 방식 
            - 적은 연산 횟수 
            - 최적의 답안이 아닌 경우 발생함
    - 동적 계획법 방식
        - 큰 문제를 작은 문제로 단순화한 후 재귀호출 활용하여 전체 문제 해결
    - 구현 [노트북](./day15/da07_dynamic_programming.ipynb)
        - 메모이제이션 배열  
        - 피보나치수열(재귀, 동적계획 비교)
            - 동적 피보나치 N의  count : N -1  
        


### 16일차 : 2월 24일  
- 검색 [노트북](./day16/da01_search.ipynb)
    - 순차검색
    - 이진검색
